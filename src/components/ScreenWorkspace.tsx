      ...
      <header className="screen-toolbar" role="group" aria-label="Screen tools">
        ...
        <PanelMenuBar label="Screen actions" menus={[
          { id: 'document', label: 'Document', items: [
            ...
            /* The Sprites workspace could always write its document into the
             * project; this one could write the generated bytes and a build
             * target, and not the screen itself. A picture imported here was
             * in this browser's storage and nowhere else, which is not what
             * a project's loading screen is. */
            { id: 'add-document', label: 'Add to project', icon: 'file', description: 'Write this editable screen into the project, where it can be opened again from this menu', hint: `${stem}.screen.json`, separated: !!openable.length, onSelect: () => onAddSource(`${stem}.screen.json`, serializeScreenDocument(document)) },
            { id: 'import-image', label: 'Import image...', icon: 'image', description: 'Convert an image into this display mode, reporting what the conversion lost', hint: 'converted', onSelect: () => imageInputRef.current?.click() },
          ] },
