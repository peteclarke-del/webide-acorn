...
describe('a screen the project already holds', () => {
  ...
  it('writes the editable screen into the project from the Document menu, named after the screen', () => {
    /* A loading screen imported from a picture was in this browser's storage
     * and nowhere else; the project could take its generated bytes and not
     * the screen itself. */
    const props = renderWorkspace();
    fireEvent.change(screen.getByLabelText('Screen name'), { target: { value: 'loading' } });
    chooseFromMenu('Document', /^Add to project/);
    expect(props.onAddSource).toHaveBeenCalledTimes(1);
    const [name, content] = props.onAddSource.mock.calls[0]!;
    expect(name).toBe('loading.screen.json');
    expect(parseScreenDocument(content).name).toBe('loading');
  });
