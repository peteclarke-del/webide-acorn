    ...
    <div className={`source-workspace source-pane-${paneId}${activePane ? ' active-pane' : ''}${inactive ? ' inactive' : ''}`} aria-label={`${paneId === 'primary' ? 'Primary' : 'Secondary'} source editor pane`} aria-hidden={inactive || undefined} inert={inactive || undefined} onPointerDown={onActivatePane} onFocusCapture={onActivatePane}>
      ...
      <div className="editor-toolbar source-toolbar">
        ...
        <div className="editor-tools" onClickCapture={(event) => {
          ...
          <details className="editor-command-menu"><summary>File</summary><div aria-label="File editor actions"><button type="button" onClick={onSave}>Save {file.name} <kbd>Ctrl S</kbd></button><button type="button" onClick={onSaveAll ?? onSave}>Save all <kbd>Ctrl ⇧ S</kbd></button><button type="button" disabled={!onRevert || !canRevertContent} onClick={revertEditor}>Revert saved content <kbd>Ctrl Alt R</kbd></button><button type="button" disabled={!onCloseFile} onClick={() => onCloseFile?.(file.id)}>Close editor <kbd>Ctrl W</kbd></button><button type="button" disabled={!onCloseOthers || files.length < 2} onClick={() => onCloseOthers?.(file.id)}>Close other editors</button><button type="button" disabled={!onCloseAll} onClick={() => onCloseAll?.()}>Close all editors</button><button type="button" disabled={!onReopenClosed || !canReopenClosed} onClick={() => onReopenClosed?.()}>Reopen closed editor <kbd>Ctrl ⇧ T</kbd></button><button type="button" disabled={file.saved === false} aria-expanded={comparisonOpen} onClick={() => setComparisonOpen((current) => !current)}>Compare saved</button>{paneId === 'secondary' ? <button type="button" onClick={onCloseSplit} title="Close the secondary source editor (Ctrl+Backslash)">Close split</button> : <button type="button" onClick={() => onRequestSplit?.(file.id)} title="Open a second independently navigable source editor (Ctrl+Backslash)">{splitOpen ? 'Reset split' : 'Split editor'}</button>}<button type="button" onClick={() => onDownloadFile(file.id)}><Icon name="download" size={14} /> Download</button><button type="button" onClick={() => onRenameFile(file.id)}>Rename</button><button type="button" onClick={() => onDeleteFile(file.id)}>Delete</button></div></details>
          <details className="editor-command-menu"><summary>Edit</summary><div aria-label="Editor actions"><button type="button" disabled={!commandHistory[file.id]?.undo.length} onClick={undoEditorCommand}>Undo command <kbd>Ctrl Z</kbd></button><button type="button" disabled={!commandHistory[file.id]?.redo.length} onClick={redoEditorCommand}>Redo command <kbd>Ctrl Y</kbd></button><button type="button" onClick={() => void cutToClipboard()}>Cut</button><button type="button" onClick={() => void copyToClipboard()}>Copy</button><button type="button" onClick={() => void pasteFromClipboard()}>Paste plain text</button><button type="button" onClick={() => selectRange(0, file.content.length)}>Select all</button><button type="button" onClick={() => runEditorCommand('duplicate-lines')}>Duplicate lines <kbd>Ctrl ⇧ D</kbd></button><button type="button" onClick={() => runEditorCommand('delete-lines')}>Delete lines <kbd>Ctrl ⇧ K</kbd></button><button type="button" onClick={() => runEditorCommand('move-lines-up')}>Move lines up <kbd>Alt ↑</kbd></button><button type="button" onClick={() => runEditorCommand('move-lines-down')}>Move lines down <kbd>Alt ↓</kbd></button><button type="button" onClick={() => runEditorCommand('join-lines')}>Join lines <kbd>Ctrl J</kbd></button><button type="button" onClick={() => runEditorCommand('split-line')}>Split line <kbd>Ctrl ⇧ Enter</kbd></button><button type="button" onClick={() => runEditorCommand('indent-lines')}>Indent lines</button><button type="button" onClick={() => runEditorCommand('outdent-lines')}>Outdent lines</button><button type="button" onClick={() => runEditorCommand('tabs-to-spaces')}>Convert tabs to spaces <kbd>Ctrl Alt T</kbd></button><button type="button" disabled={!supportsLineComments} onClick={() => runEditorCommand('toggle-comment')}>Toggle line comment <kbd>Ctrl /</kbd></button><button type="button" disabled={file.language !== 'c'} onClick={() => runEditorCommand('toggle-block-comment')}>Toggle block comment <kbd>Ctrl ⇧ /</kbd></button><button type="button" disabled={!supportsFormatting} onClick={() => runEditorCommand('format-selection')}>Format selection <kbd>Ctrl Alt F</kbd></button><button type="button" disabled={!supportsFormatting} onClick={() => runEditorCommand('format-document')}>Format document <kbd>Alt ⇧ F</kbd></button><button type="button" onClick={() => runEditorCommand('uppercase')}>Uppercase <kbd>Ctrl Alt U</kbd></button><button type="button" onClick={() => runEditorCommand('lowercase')}>Lowercase <kbd>Ctrl Alt L</kbd></button><button type="button" onClick={() => runEditorCommand('trim-trailing')}>Trim trailing whitespace <kbd>Ctrl Alt W</kbd></button><button type="button" onClick={invokeCompletion}>Suggest completions <kbd>Ctrl Space</kbd></button><button type="button" onClick={invokeSignatureHelp}>Signature help <kbd>Ctrl ⇧ Space</kbd></button></div></details>
