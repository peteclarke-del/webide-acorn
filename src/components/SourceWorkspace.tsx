...
export function SourceWorkspace({
  ...
  return (
    <div className={`source-workspace source-pane-${paneId}${activePane ? ' active-pane' : ''}${inactive ? ' inactive' : ''}`} aria-label={`${paneId === 'primary' ? 'Primary' : 'Secondary'} source editor pane`} aria-hidden={inactive || undefined} inert={inactive || undefined} onPointerDown={onActivatePane} onFocusCapture={onActivatePane}>
      ...
      <div className="editor-toolbar source-toolbar" role="group" aria-label="Editor tools">
        ...
        <div className="editor-tools" onClickCapture={(event) => {
          ...
          <details className="editor-command-menu"><summary>File</summary><div aria-label="File editor actions"><button type="button" onClick={onSave}>Save {file.name} <kbd>Ctrl S</kbd></button><button type="button" onClick={onSaveAll ?? onSave}>Save all <kbd>Ctrl ⇧ S</kbd></button><button title="Restore this file to the text and byte format last saved" type="button" disabled={!onRevert || !canRevertContent} onClick={revertEditor}>Revert <kbd>Ctrl Alt R</kbd></button><button type="button" disabled={!onCloseFile} onClick={() => onCloseFile?.(file.id)}>Close editor <kbd>Ctrl W</kbd></button><button type="button" disabled={!onCloseOthers || files.length < 2} onClick={() => onCloseOthers?.(file.id)}>Close other editors</button><button type="button" disabled={!onCloseAll} onClick={() => onCloseAll?.()}>Close all editors</button><button title="Open the source editor that was closed most recently" type="button" disabled={!onReopenClosed || !canReopenClosed} onClick={() => onReopenClosed?.()}>Reopen editor <kbd>Ctrl ⇧ T</kbd></button><button type="button" disabled={file.saved === false} aria-expanded={comparisonOpen} onClick={() => setComparisonOpen((current) => !current)}>Compare saved</button>{paneId === 'secondary' ? <button type="button" onClick={onCloseSplit} title="Close the secondary source editor (Ctrl+Backslash)">Close split</button> : <button type="button" onClick={() => onRequestSplit?.(file.id)} title="Open a second independently navigable source editor (Ctrl+Backslash)">{splitOpen ? 'Reset split' : 'Split editor'}</button>}<button type="button" onClick={() => onDownloadFile(file.id)}><Icon name="download" size={14} /> Download</button><button type="button" onClick={() => onRenameFile(file.id)}>Rename</button><button type="button" onClick={() => onDeleteFile(file.id)}>Delete</button></div></details>
          <details className="editor-command-menu"><summary>Edit</summary><div aria-label="Editor actions"><button type="button" disabled={!commandHistory[file.id]?.undo.length} onClick={undoEditorCommand}>Undo command <kbd>Ctrl Z</kbd></button><button type="button" disabled={!commandHistory[file.id]?.redo.length} onClick={redoEditorCommand}>Redo command <kbd>Ctrl Y</kbd></button><button type="button" onClick={() => void cutToClipboard()}>Cut</button><button type="button" onClick={() => void copyToClipboard()}>Copy</button><button type="button" onClick={() => void pasteFromClipboard()}>Paste plain text</button><button type="button" onClick={() => selectRange(0, file.content.length)}>Select all</button><button type="button" onClick={() => runEditorCommand('duplicate-lines')}>Duplicate lines <kbd>Ctrl ⇧ D</kbd></button><button type="button" onClick={() => runEditorCommand('delete-lines')}>Delete lines <kbd>Ctrl ⇧ K</kbd></button><button type="button" onClick={() => runEditorCommand('move-lines-up')}>Move lines up <kbd>Alt ↑</kbd></button><button type="button" onClick={() => runEditorCommand('move-lines-down')}>Move lines down <kbd>Alt ↓</kbd></button><button type="button" onClick={() => runEditorCommand('join-lines')}>Join lines <kbd>Ctrl J</kbd></button><button type="button" onClick={() => runEditorCommand('split-line')}>Split line <kbd>Ctrl ⇧ Enter</kbd></button><button type="button" onClick={() => runEditorCommand('indent-lines')}>Indent lines</button><button type="button" onClick={() => runEditorCommand('outdent-lines')}>Outdent lines</button><button title="Replace tabs in the selection with the editor tab width in spaces" type="button" onClick={() => runEditorCommand('tabs-to-spaces')}>Tabs to spaces <kbd>Ctrl Alt T</kbd></button><button type="button" disabled={!supportsLineComments} onClick={() => runEditorCommand('toggle-comment')}>Toggle line comment <kbd>Ctrl /</kbd></button><button type="button" disabled={file.language !== 'c'} onClick={() => runEditorCommand('toggle-block-comment')}>Toggle block comment <kbd>Ctrl ⇧ /</kbd></button><button type="button" disabled={!supportsFormatting} onClick={() => runEditorCommand('format-selection')}>Format selection <kbd>Ctrl Alt F</kbd></button><button type="button" disabled={!supportsFormatting} onClick={() => runEditorCommand('format-document')}>Format document <kbd>Alt ⇧ F</kbd></button><button type="button" onClick={() => runEditorCommand('uppercase')}>Uppercase <kbd>Ctrl Alt U</kbd></button><button type="button" onClick={() => runEditorCommand('lowercase')}>Lowercase <kbd>Ctrl Alt L</kbd></button><button title="Remove trailing spaces and tabs from every selected line" type="button" onClick={() => runEditorCommand('trim-trailing')}>Trim whitespace <kbd>Ctrl Alt W</kbd></button><button type="button" onClick={invokeCompletion}>Suggest completions <kbd>Ctrl Space</kbd></button><button type="button" onClick={invokeSignatureHelp}>Signature help <kbd>Ctrl ⇧ Space</kbd></button></div></details>
          <details className="editor-preferences"><summary>Preferences</summary><div><label><span>Font px</span><input aria-label="Editor font size" type="number" min={12} max={24} value={editorPreferences.fontSize} onChange={(event) => setEditorPreferences((current) => ({ ...current, fontSize: Math.max(12, Math.min(24, Number(event.target.value) || 15)), lineHeight: Math.max(current.lineHeight, Math.max(12, Math.min(24, Number(event.target.value) || 15)) + 4) }))} /></label><label><span>Line px</span><input aria-label="Editor line height" type="number" min={Math.max(16, editorPreferences.fontSize + 4)} max={44} value={editorPreferences.lineHeight} onChange={(event) => setEditorPreferences((current) => ({ ...current, lineHeight: Math.max(current.fontSize + 4, Math.min(44, Number(event.target.value) || current.fontSize + 4)) }))} /></label><label><span>Tab</span><select aria-label="Editor tab size" value={editorPreferences.tabSize} onChange={(event) => setEditorPreferences((current) => ({ ...current, tabSize: Number(event.target.value) as 2 | 4 | 8 }))}><option value={2}>2</option><option value={4}>4</option><option value={8}>8</option></select></label><label><span>Encoding</span><select aria-label={`Source encoding for ${file.name}`} value={file.encoding ?? 'utf-8'} onChange={(event) => onChangeTextFormat?.(file.id, event.target.value as SourceEncoding, file.lineEnding ?? 'lf')}><option value="utf-8">UTF-8</option><option value="utf-8-bom">UTF-8 BOM</option><option value="windows-1252">Windows-1252</option></select></label><label><span>Line endings</span><select aria-label={`Line endings for ${file.name}`} value={file.lineEnding ?? 'lf'} onChange={(event) => onChangeTextFormat?.(file.id, file.encoding ?? 'utf-8', event.target.value as SourceLineEnding)}><option value="lf">LF</option><option value="crlf">CRLF</option><option value="cr">CR</option></select></label><label><input aria-label="Editor word wrap" type="checkbox" checked={editorPreferences.wordWrap} onChange={(event) => setEditorPreferences((current) => ({ ...current, wordWrap: event.target.checked }))} /> Wrap</label><label><input aria-label="Decorate type hints beside the source" type="checkbox" checked={editorPreferences.inlayHints} onChange={(event) => setEditorPreferences((current) => ({ ...current, inlayHints: event.target.checked }))} /> Hints</label><button type="button" onClick={() => setEditorPreferences({ ...DEFAULT_EDITOR_PREFERENCES })}>Reset view</button></div></details>
          <details className="editor-command-menu source-navigation-menu"><summary>Navigate</summary><div aria-label="Source navigation actions"><button title="Jump back to the previous diagnostic in this file" type="button" disabled={!diagnosticPoints.length} onClick={() => navigateAdjacentPoint(diagnosticPoints, -1, 'diagnostic')}>Previous problem <kbd>Shift F8</kbd></button><button title="Jump forward to the next diagnostic in this file" type="button" disabled={!diagnosticPoints.length} onClick={() => navigateAdjacentPoint(diagnosticPoints, 1, 'diagnostic')}>Next problem <kbd>F8</kbd></button><button title="Jump back to the previous line that differs from the last save" type="button" disabled={!changePoints.length} onClick={() => navigateAdjacentPoint(changePoints, -1, 'saved change')}>Previous change <kbd>Alt Shift F8</kbd></button><button title="Jump forward to the next line that differs from the last save" type="button" disabled={!changePoints.length} onClick={() => navigateAdjacentPoint(changePoints, 1, 'saved change')}>Next change <kbd>Alt F8</kbd></button><button type="button" disabled={!enclosingRange} title={enclosingRange ? `${enclosingRange.label}, lines ${enclosingRange.startLine} to ${enclosingRange.endLine}` : 'No matched bracket or BASIC loop encloses the caret'} onClick={() => navigateEnclosingRange('start')}>Scope start <kbd>Alt [</kbd></button><button type="button" disabled={!enclosingRange} title={enclosingRange ? `${enclosingRange.label}, lines ${enclosingRange.startLine} to ${enclosingRange.endLine}` : 'No matched bracket or BASIC loop encloses the caret'} onClick={() => navigateEnclosingRange('end')}>Scope end <kbd>Alt ]</kbd></button>{recentFileIds.map((id) => { const recent = projectFiles.find((candidate) => candidate.id === id); return recent ? <button type="button" key={id} disabled={id === file.id} onClick={() => onSelectFile(id)}>Recent: {recent.name}{id === file.id ? ' (current)' : ''}</button> : null; })}<button type="button" onClick={toggleBookmark}><Icon name="bookmark" size={14} /> {fileBookmarks.some((bookmark) => bookmark.line === caretLineNumber) ? 'Remove bookmark' : 'Add bookmark'}</button><button type="button" disabled={!bookmarks.some((bookmark) => bookmark.enabled && !bookmark.orphaned)} onClick={() => navigateAdjacentBookmark(-1)} title="Previous enabled source bookmark">Previous bookmark</button><button type="button" disabled={!bookmarks.some((bookmark) => bookmark.enabled && !bookmark.orphaned)} onClick={() => navigateAdjacentBookmark(1)} title="Next enabled source bookmark">Next bookmark</button><button type="button" onClick={() => goToDefinition()} title="Go to an editable definition or open maintained technical research at the caret (F12)">Definition / Research</button><button type="button" onClick={() => goToRelationship('declaration')} title="Open the separately parsed declaration when available (Ctrl+F12)">Declaration</button><button type="button" onClick={() => goToRelationship('implementation')} title="Open the separately parsed implementation when available (Ctrl+Shift+F12)">Implementation</button><button type="button" onClick={() => goToRelationship('type-definition')} title="Open a connected C typedef or tagged type (Alt+F12)">Type definition</button><button type="button" onClick={() => findReferencesAtCaret()} title="Find declarations and references at the caret (Shift+F12)">References</button><button type="button" onClick={() => showCallHierarchyAtCaret()} title="Show statically parsed callers and callees at the caret (Alt+Shift+H)">Call hierarchy</button></div></details>
