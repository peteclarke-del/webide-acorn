...
type MachineMedia =
  ...
  /* A LUN image is on the BeebSCSI card rather than in a drive, so it carries a
   * LUN number and not a drive number, and nothing is ever inserted. */
          ...
          <aside className="config-panel panel-surface" aria-label="Target configuration">
            ...
            <fieldset className="capability-list">
              ...
              {machine.capabilities.map((item) => {
                 ...
                 * setting can see. */
                const unavailable = item.state === 'planned' || (!!item.requiresVariant && item.requiresVariant !== resolved.variant);
                return (
