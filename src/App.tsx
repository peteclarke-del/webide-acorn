...
import { BeebScsiWorkspace } from './components/BeebScsiWorkspace';
import { describeMountedMedia, mediaLocation, type MachineMedia } from './emulator/machineMedia';
          ...
          <aside className="config-panel panel-surface" aria-label="Target configuration">
            ...
            <fieldset className="capability-list">
              ...
              {machine.capabilities.map((item) => {
                /* A capability can be unavailable for two different reasons:
                 * it is planned, or it belongs to a variant that is not the one
                 * selected. The row is dimmed for either, so it says it is
                 * unavailable rather than leaving that to the dimming, which
                 * nobody using a screen reader or a reduced-transparency
                 * setting can see. */
                const unavailable = item.state === 'planned'
                  || (!!item.requiresVariant && item.requiresVariant !== resolved.variant)
                  || (!!item.requiresCapability && !enabledCapabilities.includes(item.requiresCapability));
                return (
                <label className={`capability-item state-${item.state}`} key={item.id} aria-disabled={unavailable || undefined}>
                  <input
                    type="checkbox"
                  ...
                  <span className="capability-copy">
                    ...
                    {item.requiresCapability && <small className="requirement-note">Needs {machine.capabilities.find((other) => other.id === item.requiresCapability)?.label ?? item.requiresCapability}{enabledCapabilities.includes(item.requiresCapability) ? '' : ', which is not enabled'}</small>}
                  </span>
                </label>
                );
              })}
    ...
    <div className="runtime-heading">
      ...
      <PanelMenuBar label={`${kind} actions`} menus={[
        ...
        { id: 'encoding', label: 'Encoding', items: [
          ...
          { id: 'encoding-screen', label: 'BBC screen order', description: 'Screen memory order: eight-scanline blocks, four-pixel columns across, and a mask a Mode 1/2 blitter can AND straight over the screen', hint: 'blitter ready', checked: document.target.packing === 'bbc-screen-2bpp-eight-line-blocks', onSelect: () => update({ target: { ...document.target, packing: 'bbc-screen-2bpp-eight-line-blocks' } }) },
        ] },
...
function MediaWorkspace({ machineId, buildArtifact, artifact, armArtifact, connected, archimedesConnected, archimedesDiscConnected, discSupported, tapeSupported, scsiSupported, mounted, onCommand, onNotice, onAnalyse }: { machineId: string; buildArtifact: BuildArtifact | null; artifact: AssemblyArtifact | null; armArtifact: ArmArtifact | null; connected: boolean; archimedesConnected: boolean; archimedesDiscConnected: boolean; discSupported: boolean; tapeSupported: boolean; scsiSupported: boolean; mounted: MachineMedia[]; onCommand: (message: Record<string, unknown>) => void; onNotice: (message: string) => void; onAnalyse: (name: string, bytes: Uint8Array, metadata: AcornFileMetadata, overrides?: { processor?: AnalysisProcessor; origin?: number; entryPoint?: number }) => void }) {
  ...
  /* LUN images are acknowledged in the BeebSCSI card section, which speaks of
   * a card rather than a drive, so they are not repeated here. */
  const acknowledgedMedia = mounted.filter((item) => item.kind !== 'scsi-lun');
  return <div className="media-workspace">
    ...
    <BeebScsiWorkspace fitted={scsiSupported} connected={connected} mounted={mounted.flatMap((item) => item.kind === 'scsi-lun' ? [{ lun: item.lun, name: item.name, size: item.size, revision: item.revision }] : [])} onCommand={onCommand} onNotice={onNotice} />
...
function EmulatorPanel({ machine, variant, machineProfile, romRecords, machineModel, romSetId, engineId, projectSettings, archimedesRuntime, romReady, tube, extraRoms, commands, artifact, state, onRun, onStep, onReset, onMachineState, onMachineMemory, onArchimedesState, onArchimedesMemory, onMachineDisassembly, onHardwareInspection, onMachineMedia, onMachineTest, onMachineError, onNotice }: EmulatorPanelProps) {
    ...
    const receive = (event: MessageEvent) => {
      ...
      if (event.data.type === 'media-loaded') {
        const media: MachineMedia = event.data.kind === 'tape'
          ...
          : event.data.kind === 'scsi-lun'
            ? { kind: 'scsi-lun', name: String(event.data.name), size: Number(event.data.size), lun: Number(event.data.drive), revision: 0 }
            : { kind: 'disc', name: String(event.data.name), size: Number(event.data.size), drive: Number(event.data.drive), dirty: false, revision: 0 };
        onMachineMedia((current) => [...current.filter((item) => media.kind === 'disc' ? !(item.kind === 'disc' && item.drive === media.drive) : media.kind === 'scsi-lun' ? !(item.kind === 'scsi-lun' && item.lun === media.lun) : item.kind !== 'tape'), media]);
      }
      ...
      if (event.data.type === 'media-exported' && event.data.kind === 'scsi-lun' && event.data.blob instanceof Blob) {
        const lun = Number(event.data.drive);
        downloadBlob(event.data.blob, safeFilename(String(event.data.name ?? `scsi${lun}.dat`)));
        onNotice(`LUN ${lun} exported · ${Number(event.data.size).toLocaleString()} bytes held · ${event.data.dirty ? `guest write revision ${Number(event.data.revision)}` : 'unwritten since it went on the card'}`);
      }
      ...
      if (event.data.type === 'media-ejected') {
        if (event.data.kind === 'scsi-lun') {
          const lun = Number(event.data.drive);
          onMachineMedia((current) => current.filter((item) => !(item.kind === 'scsi-lun' && item.lun === lun)));
          onNotice(`LUN ${lun} taken off the BeebSCSI card`);
        } else if (event.data.kind === 'disc') {
          const drive = Number(event.data.drive);
          onMachineMedia((current) => current.filter((item) => !(item.kind === 'disc' && item.drive === drive)));
  ...
  return (
    ...
    >
      ...
      />
      <div className="emulator-toolbar" role="group" aria-label="Emulator controls">
