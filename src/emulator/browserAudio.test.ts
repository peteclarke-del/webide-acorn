...
describe('browser audio test capture', () => {
  it('hashes exact sound-chip writes without requiring an audio output device', async () => {
    ...
    soundChip.poke(0x9f);
    soundChip.poke(0x80);
    expect(audio.endTestCapture()).toEqual({ digest: 'AE6CA46A', writes: 2, speakerTransitions: 0, speakerAvailable: false });
  ...
  it('resets the digest and write count for every test', async () => {
    ...
    const soundChip = audio.soundChip as unknown as { poke(value: number): void };
    audio.beginTestCapture();
    expect(audio.endTestCapture()).toEqual({ digest: '811C9DC5', writes: 0, speakerTransitions: 0, speakerAvailable: false });
    audio.beginTestCapture();
    soundChip.poke(0);
    expect(audio.endTestCapture()).toEqual({ digest: '050C5D1F', writes: 1, speakerTransitions: 0, speakerAvailable: false });
    await audio.close();
  });
