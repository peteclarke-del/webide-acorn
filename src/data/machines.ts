...
export const machineProfiles: MachineProfile[] = [
  ...
  },
  {
    ...
    capabilities: [
      ...
      capability('tube', 'Tube second processor', 'External 6502 parasite. OS 1.20 finds the Tube and stops there, so the Tube host code has to be in a sideways bank: Acorn shipped it in DNFS, and the ROM set asks for it when this is switched on. With it the machine introduces itself as Acorn TUBE 6502 64K', 'supported'),
      capability('tube-turbo', '65C102 Turbo second processor', 'Which processor sits behind the Tube, rather than a second Tube: a 65C102 at 4 MHz instead of the 6502 at 3. Acorn sold this board for the Master and not for this machine, but a PiTube Direct puts one behind whatever it is plugged into, and the parasite ROM introduces itself as Acorn TUBE 65C102 Co-Processor here too', 'supported', false, 'the Tube interface', undefined, 'tube'),
      capability('tube-z80', 'Z80 second processor', 'What Acorn sold for CP/M, and emulation 5 on a PiTube Direct. Its parasite ROM is identified and reads Acorn TUBE Z80 64K 1.20; the pinned core has no Z80 at all, so running it needs a processor this build does not have', 'planned', false, 'a Z80 core behind the Tube', undefined, 'tube'),
      capability('tube-arm2', 'ARM2 second processor', 'The Acorn ARM Evaluation System, and emulation 12 on a PiTube Direct. Its parasite ROM is identified and is the Brazil supervisor of August 1986. This build has an ARM2 inside the pinned Arculator, but that is a whole Archimedes rather than a core that can sit behind a Tube', 'planned', false, 'an ARM2 core behind the Tube', undefined, 'tube'),
      capability('tube-32016', '32016 second processor', 'The Acorn Cambridge Co-processor, running Panos, and emulation 13 on a PiTube Direct. No 32016 core is present here and no parasite ROM for it is identified', 'planned', false, 'a 32016 core behind the Tube', undefined, 'tube'),
      capability('tube-80286', '80286 second processor', 'Emulation 8 on a PiTube Direct, and the closest it offers to the 80186 Acorn put inside the Master 512 for DOS Plus. No x86 core is present here and no parasite ROM for it is identified', 'planned', false, 'an x86 core behind the Tube', undefined, 'tube'),
      capability('econet', 'Econet', 'Network interface and NFS ROM', 'planned'),
  ...
  },
  {
    ...
    capabilities: [
      ...
      capability('beebscsi', 'BeebSCSI', 'Acorn SCSI host adapter at &FC40 with a SCSI-1 drive behind it. LUN images on a card, reached by ADFS, and the fast loads that come with them', 'preview'),
      capability('tube', 'Tube second processor', 'External 6502 parasite. MOS 2.00 carries the Tube host code itself, so a B+ needs nothing in a bank the way a Model B does, and introduces itself as Acorn TUBE 6502 64K', 'supported'),
      capability('tube-turbo', '65C102 Turbo second processor', 'Which processor sits behind the Tube, rather than a second Tube: a 65C102 at 4 MHz instead of the 6502 at 3. Acorn sold this board for the Master and not for this machine, but a PiTube Direct puts one behind whatever it is plugged into, and the parasite ROM introduces itself as Acorn TUBE 65C102 Co-Processor here too', 'supported', false, 'the Tube interface', undefined, 'tube'),
      capability('tube-z80', 'Z80 second processor', 'What Acorn sold for CP/M, and emulation 5 on a PiTube Direct. Its parasite ROM is identified and reads Acorn TUBE Z80 64K 1.20; the pinned core has no Z80 at all, so running it needs a processor this build does not have', 'planned', false, 'a Z80 core behind the Tube', undefined, 'tube'),
      capability('tube-arm2', 'ARM2 second processor', 'The Acorn ARM Evaluation System, and emulation 12 on a PiTube Direct. Its parasite ROM is identified and is the Brazil supervisor of August 1986. This build has an ARM2 inside the pinned Arculator, but that is a whole Archimedes rather than a core that can sit behind a Tube', 'planned', false, 'an ARM2 core behind the Tube', undefined, 'tube'),
      capability('tube-32016', '32016 second processor', 'The Acorn Cambridge Co-processor, running Panos, and emulation 13 on a PiTube Direct. No 32016 core is present here and no parasite ROM for it is identified', 'planned', false, 'a 32016 core behind the Tube', undefined, 'tube'),
  ...
  },
  {
    ...
    capabilities: [
      ...
      capability('plus3', 'Plus 3 expansion', '3.5-inch disk and ADFS', 'preview', false, 'one filing system ROM, ADFS or the Electron DFS, in the firmware vault. Fitting one was measured: with ADFS alone supplied through the workbench and the capability switched on, the Elkulator core reports three ROM images and no longer names adfs.rom as absent. What that proves is that the interface has its filing system, not that a disc has been mounted or read through it, which is why this is preview rather than supported. Mounting a disc image through the bridge and reading it back has not been done'),
