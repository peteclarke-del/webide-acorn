...
const BEEBSCSI_CITATION = [{ title: 'BeebSCSI Technical Guide', url: 'https://github.com/simoninns/BeebSCSI/wiki/Technical-Guide', section: 'Host adapter implementation' }, { title: 'BeebSCSI host adapter CPLD (hostAdapter.v)', url: 'https://github.com/simoninns/BeebSCSI/blob/master/BeebSCSI_CPLD/hostAdapter.v', section: 'Address decoding and status byte' }];
...
const HARDWARE: HardwareRecord[] = [
  ...
  { token: 'BEEBSCSI_DATA', address: 0xfc40, detail: 'Acorn SCSI host adapter data register. Reads take a byte from the SCSI bus and writes put one on it, and either way the access clocks ACK. While the drive is not driving the bus a read returns the last byte written, which is how ADFS tests that the adapter is fitted.', machines: ['bbc-b', 'bbc-bplus', 'master'], citations: BEEBSCSI_CITATION },
  { token: 'BEEBSCSI_STATUS', address: 0xfc41, detail: 'Acorn SCSI host adapter status. Bit 7 C/D, bit 6 I/O, bit 5 REQ, bit 4 IRQ, bit 1 BSY, bit 0 MSG. Bits 2 and 3 read as zero on the external bus.', machines: ['bbc-b', 'bbc-bplus', 'master'], citations: BEEBSCSI_CITATION },
  { token: 'BEEBSCSI_SELECT', address: 0xfc42, detail: 'Acorn SCSI host adapter select. Any write asserts SEL, which selects the drive and starts the command phase. The byte on the bus is the host identifier.', machines: ['bbc-b', 'bbc-bplus', 'master'], citations: BEEBSCSI_CITATION },
  { token: 'BEEBSCSI_IRQ_ENABLE', address: 0xfc43, detail: 'Acorn SCSI host adapter interrupt enable. Bit 0 set lets REQ pull the host IRQ line down; writing zero clears the latch and the interrupt.', machines: ['bbc-b', 'bbc-bplus', 'master'], citations: BEEBSCSI_CITATION },
  { token: 'BEEBSCSI_CONFIG', address: 0xfc44, detail: 'BeebSCSI configuration byte, which is not part of the Acorn adapter. It steers the board debug output and does not touch the SCSI command flow. Reachable from the operating system as *FX 147,68,n.', machines: ['bbc-b', 'bbc-bplus', 'master'], citations: BEEBSCSI_CITATION },
