...
const directives = [
  ...
  item('EQUD', 'directive', 'Emit comma-separated little-endian 32-bit double words, as an OSFILE control block or a Tube-side address wants.', 'EQUD value[, value...]', ['6502']),
...
const beebAsmDirectives: LanguageItem[] = [
  ...
  { token: 'INCLUDESCREEN', kind: 'directive', detail: 'Generate the packed frame buffer of a versioned screen document at this address.', signature: 'INCLUDESCREEN "name.screen.json" [RLE]', parameters: ['name.screen.json', 'RLE'], languages: ['6502'], insertText: 'INCLUDESCREEN ""', source: { kind: 'builtin', label: '8bit-net screen build adapter', version: '1' }, documentation: { category: '8bit-net generated screen dependency', parameters: [{ name: 'name.screen.json', detail: 'A project-local schema-1 full-screen bitmap document.' }, { name: 'RLE', detail: 'Emit the frame buffer run-length packed under screen_<name>_rle: a count of 1 to 255 then the byte, ended by a count of 0, for the program to unpack.' }], examples: ['INCLUDESCREEN "title.screen.json"', 'INCLUDESCREEN "title.screen.json" RLE'], sideEffects: ['Emits the whole frame buffer in hardware character-block order: 10,240 bytes for MODE 4 or 5 and 20,480 for MODE 0, 1 or 2; with RLE, the packed form instead, which for a screen that is mostly one colour is a few hundred bytes.', 'Adds the screen document to provenance, cache keys, incremental impact and stale-artifact checks.'], compatibility: { supported: true, appliesTo: ['8bit-net.browser-6502'], warning: 'The bytes are the picture only; select the mode and the palette separately.' } } },
