...
export const HELP_TOPICS: HelpTopic[] = [
  ...
  {
    id: "emulator-tube-parasite",
    category: "Run",
    title: "Fit a second processor and choose which one",
    summary:
    ...
    steps: [
      "Open the target bar and find the capabilities for the selected machine.",
      "Choose Tube second processor, which fits the board Acorn sold with that machine: a 6502 for a BBC B or B+, a 65C102 for a Master.",
      "Add 65C102 Turbo second processor alongside it to put a 65C102 behind the Tube of a BBC B or B+ instead. It is which processor rather than a second Tube, so it needs the Tube capability on as well and is disabled until it is.",
      "Supply the parasite boot ROM the firmware vault now asks for. It is 2 KiB and differs between the two processors.",
  ...
  },
  {
    id: "emulator-beebsid",
    category: "Run",
    title: "Fit a BeebSID and hear it",
    summary:
    ...
    steps: [
      "Enable BeebSID in the target profile. It is a 1 MHz bus board, so it appears on every BBC-family machine that has that bus.",
      "Start the machine and enable sound.",
      "Write to the chip at &FC20 upwards. The register file is the 6581's own: seven registers per voice, then the filter and volume.",
      "Open the fourth voice register to open a gate. The chip is silent until one opens, whatever else has been written.",
      "Use the machine's own SOUND and ENVELOPE for effects at the same time. The two are mixed rather than exchanged.",
    ],
    expected: [
  ...
  },
  {
    ...
    steps: [
      "Enable BeebSCSI in the target profile. It is a 1 MHz bus board, so it appears on every BBC-family machine that has that bus.",
      "Choose the ADFS ROM set for the machine, then start it.",
      "Open Media and find BeebSCSI card.",
      "Choose the LUN. ADFS reaches LUNs 0 to 3; VFS reaches all eight, and the drive answers for eight.",
      "For an image you already have, choose it under Image. A file named scsi0.dat or similar selects its own LUN.",
      "Supply the matching .dsc under Descriptor when you have one. Without it the board builds the descriptor from the size of the image, which is what the real board does.",
      "Read the cylinders, heads, sectors and capacity, and any warnings, before putting it on the card.",
      "Choose Put on card. The image appears under the card listing once the runtime has it.",
      "For a new disc, choose a size and Create blank LUN. It arrives unformatted and holds no sectors until something writes to it.",
      "At the machine, use ADFS as normal. *MOUNT starts a LUN and *BYE stops it, and a read of a stopped LUN that has an image starts it.",
      "Choose Export current to download the image as it now stands, with whatever the machine wrote into it.",
      "Choose Take off card to remove it. Nothing is ejected: the file leaves the card.",
    ],
    expected: [
    ...
    screenshot: {
      src: "/help/emulator-beebscsi-card.png",
      alt: "BBC Model B Media workspace showing the BeebSCSI card section with a blank LUN created and its cylinders, heads, sectors and capacity listed",
      caption:
  ...
  },
  {
    ...
    steps: [
      ...
      "To open a stored project as the project, use Start a project and its From the project store tab, or run Project store: open a project from the command palette. Project store: save this project writes a revision without leaving the editor.",
    ],
    expected: [
