...
| Measure | Count |
| --- | --- |
| Requirements tracked | 333 |
| Complete | 244 |
| Complete and traced | 157 |
...
| P0-005 | Define analytics/feedback needs with privacy minimization; analytics... | the `browsers` and `smoke` gate stages load the built workbench under its shipped content security policy, whose `connect-src` is `'self'`, and fail on any policy violation; so a dependency that began calling out would... |
| P0-012 | Prototype, not productize, emulator contracts for pause, instruction... | the adapter contract tests for each core hold its declared capabilities to what its runtime actually implements, 10 for ElkJS, 13 for Elkulator including that every offered capability names a bridge entry point that... |
| P0-013 | Inventory toolchains for BeebAsm-compatible 6502, general 6502/... | `docs/toolchain-evaluation.md` carries the scored, primary-source comparison of the 6502 candidates; ADRs 0003, 0004, 0005 and 0009 record each adopted or rejected toolchain with its boundary; and the `backend` gate... |
| P0-015 | Inventory media and asset format implementations and run malicious-... | 156 contracts across `src/media`, of which 27 assert a refusal by name. A length that is no disc, a signature that is missing, a catalogue that points off the disc, a directory that loops, a half-written directory, an... |
| P0-016 | Produce ROM/firmware matrix by profile: required/optional ROM,... | 6 contracts in `src/rom/firmwareMatrix.test.ts`, including that the document matches what the manifests generate, that every ROM and every vault key appears, that every optional ROM names the capability that needs it,... |
| P0-020 | Define and validate the machine-profile JSON schema, inheritance/... | 17 contracts in `src/profiles/profileManifest.test.ts` and 19 in `src/profiles/profileRegistry.test.ts`, including the round trip of a shipped profile without change, each refusal by name, and that a malformed accent... |
| P0-021 | Model Atom variants and legal ROM/storage prerequisites. | 3 contracts in `src/data/machines.test.ts` for the catalogue itself and 19 in `src/profiles/profileRegistry.test.ts` for resolution, among them that a peripheral fitted only to a variant that is not selected is refused,... |
| P0-024 | Model base Electron, Plus 1, Plus 3, memory and storage expansions;... | 12 contracts in `src/rom/romProfiles.test.ts`, 5 of which cover the Electron expansion set (its pinned engine, the required-versus- gated split, the accepted sizes, the boards covered and where each ROM mounts), with... |
| P0-026 | Model ARM2/ARM3 Archimedes groups only after verifying equivalent... | 10 contracts in `src/rom/adapterSupport.test.ts`, including that the A310 alone is qualified and that a machine with no engine is distinguished from one on another platform class, and 6 in... |
| P0-027 | Model later ARM Acorn profiles in a distinct compatibility tier. | among the 10 contracts in `src/rom/adapterSupport.test.ts`, one asserts every ARM machine outside the qualified class reports no engine and carries the tier's own wording, and `src/data/compatibilityMatrix.test.ts`... |
| P0-028 | Model sideways ROM/RAM slots, shadow/private/hazel memory, ROM... | 20 contracts in `src/rom/sidewaysSlots.test.ts` for the banks and their priority, 3 in `src/data/machines.test.ts` for the catalogue, and 19 in `src/profiles/profileRegistry.test.ts` for what happens when a capability... |
| P0-040 | Draw system context and container diagrams separating browser,... | `scripts/documentation.test.ts` holds the architecture document to naming every module directory that exists and none that does not, and to linking only documents that exist, so a diagram describing a container that was... |
| P0-041 | Define bounded contexts and domain contracts from API-001. | 10 contracts in `src/api/contracts.test.ts`, including that no `/api/` literal exists anywhere in the product outside the generated module and that no declared route goes uncalled; the backend conformance test drives... |
| P0-042 | Write ADR for persistence using access patterns, consistency,... | 26 contracts in `backend/tests/Storage/ProjectStoreTest.php` for the access patterns and the stale-parent refusal, 5 in `backend/tests/Storage/StoreRecoveryTest.php` for backup, restore and integrity, and the... |
| P0-046 | Define build/runtime sandbox: non-root identity, immutable image,... | 6 contracts in `scripts/sandboxDeployment.test.ts` audit the shipped Compose definition for exactly these properties, so a relaxation made in the deployment fails the gate rather than passing unnoticed, and the backend... |
...
| MED-309 | Fuzz all parsers and property-test round trips, lossy-conversion... | 29 contracts in `src/media/mediaParserProperties.test.ts`. No parser crashed or returned a malformed structure on any generated input. The generator built each case with `Uint8Array.from({ length }, ...)`, which goes... |
...
| PRJ-208 | Opening a folder that held one of the workbench's own sprite... | two contracts in `src/project/codebaseImport.test.ts`, one with a 256-line pixel array that is offered as no map and still imported as a file, one with a drawn room requested at a shape it does not allow that is left... |
| AST-631 | The Screens workspace could write the generated bytes of a screen and... | a contract in `src/components/ScreenWorkspace.test.tsx`: naming a screen `loading` and choosing Add to project writes `loading.screen.json` whose document parses back with that name. |
...
| EMU-440 | Two commands for the machine queued in one tick lost the first: the... | the FireWing demonstration boots the game with Build and boot, which needs both commands to arrive. |
| AST-635 | A song could not be heard where it was written. The Sound workspace... | `src/assets/songPlayback.test.ts` for the pitch scales, the rows and the transport; `src/assets/songPlayer.test.ts` for what is asked of the browser's audio; `src/components/SongWorkspace.test.tsx` plays, steps, pauses,... |
...
| OPS-902 | Benchmark application startup, edit latency, live diagnostics, build,... | 11 contracts in `scripts/benchmarks.test.ts`, including each of the three kinds of finding produced deliberately (a figure over its ceiling, a case that produced nothing, and a case that is absent), because a check that... |
...
| DOC-901B | Capture maintained screenshots from the shipped interface for all... | The release gate's help stage fails on a missing screenshot file, a broken topic link or a control name that no longer exists in the interface, so a stale screenshot cannot survive a rename. showed something the reader... |
...
| P0-003 | Record the exact public-release target profiles and the definition of "supports the Acorn... | 2 of 3 parts done |
| P0-004 | Convert the Web64 baseline table into demonstration scripts and acceptance scenarios;... | not started |
| P0-010 | Inventory candidate emulators for Atom, BBC/Electron/Master, Tube, ARM Archimedes, and... | 1 of 2 parts done |
| P0-011 | For each candidate, record repository/upstream, maintainer health, licence and transitive... | 2 of 3 parts done |
| P0-014 | Prove diagnostic parsing, label/source-map generation, deterministic output,... | 4 of 4 parts done |
| P0-017 | Produce manuals/reference-content matrix with title, owner, version, applicable targets,... | 2 of 3 parts done |
| P0-018 | Complete dependency licence/security review and approve selected candidates. Unresolved... | 4 of 5 parts done |
| P0-022 | Model BBC A/B, DFS controller/filing-system differences, common regional variants, input,... | 3 of 4 parts done |
| P0-023 | Model B+ 64/128 shadow and sideways memory accurately. | 1 of 2 parts done |
| P0-025 | Model Master 128, Turbo, 512, and Compact as host/expansion profiles rather than names... | 2 of 3 parts done |
| P0-029 | Define host-plus-parasite Tube schema, clocks/address spaces, ULA channels, emulator... | 2 of 3 parts done |
| P0-030 | Review every profile with machine specialists and cite technical sources; create... | not started |
| P0-043 | Write ADRs for job/event delivery, object storage, real-time debug transport,... | 2 of 3 parts done |
| P0-044 | Define API schemas, error envelope, idempotency, pagination, binary transfer, job state... | 5 of 6 parts done |
| P0-045 | Threat-model all assets and boundaries in SEC-001 and assign controls, test strategy,... | 2 of 3 parts done |
...
| ANL-309 | Add Atom BASIC and BBC BASIC I/IV/V/VI dialect adapters and golden... | 17 of 18 parts done |
