...
| Build target | `build-target.schemaVersion` | 6 | Migrated forward when a project document is opened. A target that names a toolchain this build does not register keeps its declaration and is reported as unavailable rather than silently reassigned. |
