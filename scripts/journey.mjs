...
const WALK = `(async (machineId, templateId, expectRunnable) => {
   ...
   */
  await chooseFromMenu('Workbench menu', 'Assets', /^Sprites$/, 'open the Sprites editor');
   ...
   */
  await chooseFromMenu('Workbench menu', 'Assets', /^Sound$/, 'open the Sound editor');
  ...
  await chooseFromMenu('Workbench menu', 'Workspace', /^Build targets$/, 'open the Build targets workspace');
  await wait(400);
  ...
  await chooseFromMenu('Workbench menu', 'Workspace', /^Media$/, 'open the Media workspace');
  await wait(600);
